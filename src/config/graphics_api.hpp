...
namespace GAPI {
	...
	void MakeWindow(std::string szWindowTitle, std::string szWindowClass,
		...
		WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(0), 0, 0, 0, 0, szWindowClass.c_str(), 0 };
		::RegisterClassEx(&wc);

		g_hwnd = ::CreateWindowA(wc.lpszClassName, szWindowTitle.c_str(), WS_POPUP | WS_VISIBLE, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), 0, 0, wc.hInstance, 0);
		...
		std::thread([]() {
			...
			main_font = io.Fonts->AddFontFromMemoryCompressedTTF(Pixel_compressed_data, Pixel_compressed_size, 12.f, NULL, io.Fonts->GetGlyphRangesJapanese());
